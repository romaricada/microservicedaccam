...
import mena.gov.bf.service.CautionService;
...
/**
 * REST controller for managing {@link mena.gov.bf.domain.Caution}.
 */
@RestController
@RequestMapping("/api")
public class CautionResource {
    ...
    private static final String ENTITY_NAME = "microservicedaccamCaution";
    ...
    @PutMapping("/cautions/deleteAllListe")
    public ResponseEntity<List<CautionDTO>> deleteAll(
        @RequestBody List<CautionDTO> cautionDTOS) {
        return ResponseEntity.ok(cautionService.deleteAllCaution(cautionDTOS));
    }


    @GetMapping("/receptions/caution-by-lot")
    public ResponseEntity<List<CautionDTO>> findAllByLot(@RequestParam Long lotId) {
        return ResponseEntity.ok(cautionService.findCautionByLot(lotId));

    }
    @GetMapping("/cautions/caution-by-type")
    public ResponseEntity<List<CautionDTO>> findAllCautionByTypeCaution(@RequestParam Long typeCautionId) {
        log.debug("REST request to get alla by type Caution : {}", typeCautionId);
        return ResponseEntity.ok(  ).body( cautionService.findAllCautionByTypeCaution(typeCautionId) ) ;
    }
    @GetMapping("/cautions/caution-by-lot")
    public ResponseEntity<List<CautionDTO>> findAllCautionByLotId(@RequestParam Long lotId) {
        log.debug("REST request to get alla by type Caution : {}", lotId);
        return ResponseEntity.ok().body( cautionService.findAllCautionByLotId(lotId) ) ;
